...
private:
	std::vector<std::triple<dist_type, double, double>> prior_distributions;	// stores information about the prior distributions (type and parameter) - one for each variable
	unsigned dim_data;
...
public:
	...
	void set_prior_distributions(const std::vector<std::triple<dist_type, double, double>> &pridist);	// sets the types and parameters for each prior distribution (one for each variable)
