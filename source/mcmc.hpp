...
// accepting proposal
	virtual bool success(const std::vector<std::vector<double>> &observations);	// returns 1 if update is accepted, 0 otherwise. has to be specified in inherited class!
...
public:
	...
	void update(const std::vector<std::vector<double>> &observations);				// proposes new position and accepts it if success evaluates to true
};
